...
(function () {
  ...
  /**
   * Let somebody say they are coming, and how many.
   *
   * The page never learns anybody's email address. It sends the event id and
   * what was typed; the endpoint looks up who to tell. That keeps every leader's
   * address off a public page which also happens to carry a form.
   *
   * Answering twice replaces the first answer rather than adding a second, so
   * changing your numbers is just filling it in again.
   */
  /*
   * What this browser has already said it is coming to.
   *
   * The server has always been safe: answering twice under the same name
   * replaces the first answer rather than adding a second. What was missing was
   * the reader knowing that. Refresh the page and "You're down for 4" vanished,
   * so the honest thing for somebody to do was fill it in again — and wonder
   * whether they had just double-booked their family.
   *
   * Per browser, deliberately. Recognising a returning person any other way
   * means asking who they are before they have said anything, and this form's
   * whole merit is that it asks for a name and a number and nothing else.
   */
  const RSVP_KEY = "glbc.rsvps.v1";

  function rsvpKey(e){ return e.uid + "|" + e.start; }
  ...
  function rememberRsvp(e, name, count){
    try {
      const all = myRsvps();
      all[rsvpKey(e)] = { name, count, at: Date.now() };
      // Only what is still ahead. A year of past events would otherwise sit in
      // storage forever for no benefit.
      const alive = {};
      const cutoff = Date.now() - 24 * 60 * 60 * 1000;
      for (const [k, v] of Object.entries(all)){
        const when = Date.parse(k.split("|")[1]);
        if (!Number.isFinite(when) || when > cutoff) alive[k] = v;
      }
      localStorage.setItem(RSVP_KEY, JSON.stringify(alive));
    } catch {
      // Private browsing, or storage switched off. The RSVP still went through;
      // this reader simply will not be recognised when they come back.
    }
  }

  function forgetRsvp(e){
    try {
      const all = myRsvps();
      delete all[rsvpKey(e)];
      localStorage.setItem(RSVP_KEY, JSON.stringify(all));
    } catch {}
  }

  function addRsvp(card, e){
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "rsvpbtn";
    card.appendChild(btn);

    const mine = myRsvps()[rsvpKey(e)];
    if (mine){
      btn.classList.add("done");
      btn.textContent = "You're down for " + mine.count + " — change it";
    } else {
      btn.textContent = "I'm coming";
    }

    btn.addEventListener("click", () => {
      if (card.querySelector(".rsvpform")) return;
      btn.hidden = true;

      const form = document.createElement("div");
      form.className = "rsvpform";
      form.innerHTML =
        '<p class="ask">Let ' + esc(e.contact) + ' know you are coming.</p>' +
        '<div class="fields">' +
          '<label>Your name<input type="text" class="r-name" maxlength="80" autocomplete="name"></label>' +
          '<label>How many<input type="number" class="r-count" min="1" max="99" value="1" inputmode="numeric"></label>' +
          '<label class="wide">Phone <span style="font-weight:400">(optional)</span>' +
            '<input type="tel" class="r-phone" maxlength="30" autocomplete="tel"></label>' +
          '<label class="wide">Anything they should know <span style="font-weight:400">(optional)</span>' +
            '<input type="text" class="r-note" maxlength="300"></label>' +
        '</div>' +
        '<div class="go"><button type="button">Send</button><span class="st"></span></div>';
      card.appendChild(form);
      if (mine){
        form.querySelector(".r-name").value = mine.name || "";
        form.querySelector(".r-count").value = mine.count || 1;
      }
      form.querySelector(".r-name").focus();

      const st = form.querySelector(".st");
      const send = form.querySelector("button");
      send.addEventListener("click", async () => {
        const name = form.querySelector(".r-name").value.trim();
        const count = parseInt(form.querySelector(".r-count").value, 10);
        if (name.length < 2){ st.className = "st err"; st.textContent = "Please put your name."; return; }
        if (!(count >= 1)){ st.className = "st err"; st.textContent = "How many are coming?"; return; }

        send.disabled = true;
        st.className = "st";
        st.textContent = "Sending…";
        try {
          const res = await fetch(DATA.signup, {
            method: "POST",
            ...
            body: JSON.stringify({
              ...
              name: name,
              count: count,
              phone: form.querySelector(".r-phone").value.trim(),
              note: form.querySelector(".r-note").value.trim(),
            }),
          });
          const out = await res.json();
          if (!out.ok) throw new Error(out.error || "That did not go through.");
          rememberRsvp(e, name, count);
          form.remove();
          btn.hidden = false;
          btn.classList.add("done");
          btn.textContent = "You're down for " + count + " — change it";
        } catch (err) {
          st.className = "st err";
          st.textContent = (err && err.message) || "Could not reach the church server.";
          send.disabled = false;
        }
      });
    });
  ...
  function renderAgenda(){
    ...
    list.forEach(e => {
      ...
      row.innerHTML =
        ...
        '</div>';
      // Only where somebody is named to answer to, and only for the endpoint
      // that records it. An RSVP button with nowhere to send is worse than none.
      if (e.contact && DATA.signup) addRsvp(row.querySelector(".card"), e);
